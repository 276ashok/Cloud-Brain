...
if(isset($btn))
{
$qry = mysqli_query($connect,"select max(id) as maxid from vb_document");
$rs=mysqli_fetch_array($qry);
$id = $rs['maxid']+1;

	if($_FILES['file']['name']!="")
	{
$fname="D".$id.$_FILES['file']['name'];
move_uploaded_file($_FILES['file']['tmp_name'],"document/".$fname);
$originalfile="document/$fname";
$crypt="document/$fname";
Cryptfile($originalfile,$crypt,$key);
	}
	else
	{
	$fname="";
...
$ins=mysqli_query($connect,"insert into vb_document(id,uname,title,details,filename,rdate) values($id,'$uname','$title1','$details1','$fname1','$rdate1')");					
...
{
$did=$_REQUEST['did'];
$qq1=mysqli_query($connect,"select * from vb_document where id=$did");
$rr1=mysqli_fetch_array($qq1);
$fn=$rr1['filename'];
@unlink("document/$fn");
mysqli_query($connect,"delete from vb_document where id=$did");
header("location:document.php");
}
//////
if($act=="down")
		...
		<h3>Your Documents</h3>
			<div class="row">
				<div class="col-lg-6">
					...
					<div class="col-md-8">
								...
								<label>Title of Document</label>
									<input class="check-form" type="text" name="title" placeholder="">
									<span><i class="ti-check"></i></span>
								</div>
							</div>
							<div class="col-md-8">
								<div class="form-group">
								<label>Details about document</label>
									<textarea class="check-form" name="details"></textarea>
									<span><i class="ti-check"></i></span>
								</div>
							</div>
						<div class="col-md-8">
				...
				<div class="col-lg-6 mt-5 mt-lg-0">
					...
					<?php
		  $q1=mysqli_query($connect,"select * from vb_document where uname='$uname'");
		  ...
		  ?>
						<div class="row">
							...
							<div class="col-md-6">
								...
								$ff=fnDecrypt($r1['filename'],$key);
			  echo '<a href="document.php?act=down&fid='.$r1['id'].'">'.$ff.'</a>'; 
			  } else { echo "None"; } ?>
							</div>
							<div class="col-md-6">
								Relative
			  ...
			  ?>
							...
							<p align="right"><a href="edit_doc.php?act=doc&amp;id=<?php echo $r1['id']; ?>">Edit</a> | <a href="document.php?act=del&amp;did=<?php echo $r1['id']; ?>" onClick="return del()">Delete</a></p>
							</div>
						</div>
